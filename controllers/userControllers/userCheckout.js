      ...
      for (const product of cart.products) {
        ...
        if (
          ...
          res.status(200).json({
            success: false,
            message:
              "Some products were removed from your cart as they are either blocked or out of stock.",
          });
  ...
  try {
    ...
    if (validProducts.length === 0) {
      res.status(200).json({
        success: false,
        message:
          "Your cart contains blocked or out-of-stock products. Please update your cart.",
      });
      setTimeout(() => {
        return res.redirect("/cart");
      }, 2000);
...
const verifyCoupon = async (req, res) => {
  try {
    ...
    if (coupon.usedCount >= coupon.usageLimit) {
      return res
        .status(200)
        .json({ success: false, message: "Usage limit completed" });
    }
    if (cart.totalPrice >= coupon.maxPurchaseValue) {
      return res.status(200).json({
        success: false,
        message: `Total price is more than ${coupon.maxPurchaseValue}`,
      });
    }
    if (cart.totalPrice < coupon.minPurchaseValue) {
      return res.status(200).json({
        success: false,
        message: `Total price is less than ${coupon.minPurchaseValue}`,
      });
    }
...
const cancelOrder = async (req, res) => {
  try {
    ...
    if (order.orderStatus !== "Cancelled") {
      ...
      const changeOrder = await Order.findByIdAndUpdate(orderId, {
