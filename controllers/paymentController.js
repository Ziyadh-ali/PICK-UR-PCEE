...
const convertCurrency = async (amountInINR) => {
  try {
    ...
    const fallbackAmount = (amountInINR * fallbackRate).toFixed(2);

    console.log("Using fallback USD:", fallbackAmount);
...
const payWithPaypal = async (req, res) => {
  try {
    ...
    console.log("Step 2: Total Price", totalPrice);

    let total = await convertCurrency(totalPrice);
    console.log("Step 3: Converted USD total", total);
    ...
    console.log("Step 4: Payment JSON ready", create_payment_json);
    ...
    paypal.payment.create(create_payment_json, function (error, payment) {
      ...
      for (let i = 0; i < payment.links.length; i++) {
        if (payment.links[i].rel === "approval_url") {
          return res.json({ success: true, redirectUrl: payment.links[i].href });
        }
      }
      res.status(400).json({ success: false, message: "Approval URL not found in PayPal response" });
    });
  } catch (error) {
